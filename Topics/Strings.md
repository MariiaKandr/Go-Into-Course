# Строки
Строки применяются в разных контекстах: от простых приветствий вроде «Hello, World!» до сложных парсеров, логгеров и сетевых протоколов. Строки используются в объявлениях, манипуляциях и кодировании, а также в работе с библиотеками для работы со строками. Фактически мы говорим **обо всех** уровнях приложений.
Во-первых, всё сетевое общение в интернете — это строки (URL, заголовки, JSON/XML bodies, cookies).

<img width="884" height="340" alt="image" src="https://github.com/user-attachments/assets/5715e7c0-d098-4a23-ba26-42dfe2ac0138" />

HTTP-запрос — это **сообщение**, которое одно устройство (обычно браузер, мобильное приложение или любой клиент) отправляет на сервер, чтобы попросить у него **данные** или попросить его **что-то сделать**.
...
* **GET** — «дай данные»
* **POST** — «вот данные, сохрани»
* **PUT** — «обнови»
* **DELETE** — «удали»


### 2) **URL (адрес)** — куда мы обращаемся
...
* `/api/users` — путь
* `?id=10` — query-параметр

### 3) **Заголовки (headers)** — дополнительная информация

Например:

* `User-Agent` — кто отправил запрос (браузер, приложение)
* `Content-Type` — тип данных (JSON, текст)
* `Authorization` — токен доступа

### 4) **Тело (body)** — данные, которые клиент отправляет
...
Тело есть только в POST/PUT/PATCH (обычно).

Когда Вы в браузере вводите адрес:
...
Сервер отвечает HTML-страницей — это **HTTP-ответ**.


```go
// Обработка HTTP запроса 
func handler(w http.ResponseWriter, r *http.Request) {
    // Получаем путь запроса как строку, например "/search"
    path := r.URL.Path

    // Читаем query-параметр ?q=... как строку (может содержать Unicode)
    q := r.URL.Query().Get("q")

    // Берём строку User-Agent из заголовков (кто отправил запрос)
    userAgent := r.Header.Get("User-Agent")

    // Читаем всё тело запроса как массив байтов
    bodyBytes, _ := io.ReadAll(r.Body)

    // Превращаем []byte в строку — это создаёт копию данных
    bodyStr := string(bodyBytes)

    // Печатаем полученные данные и длину тела в байтах
    fmt.Println(path, q, userAgent, len(bodyStr))
}
```

---

Авторизация как основная операция для подтверждения личности пользователей использует **JWT, cookies, токены** - как правило уникальные строки.

```go
authHeader := r.Header.Get("Authorization")
if strings.HasPrefix(authHeader, "Bearer ") {
    token := strings.TrimPrefix(authHeader, "Bearer ")
    // token — строка; дальше парсим/валидируем библиотекой
}
```
---
В целом данные внутри инфомрационной системы как правило хранятся в структурированном виде в таблицах (базах данных), а потому запросы на извлечение, фильтрация и ключи кэширования - все это не что иное как строки. Здесь появляется связка с еще одним инструментальным средством - SQL (Structured Query Language) — языком структурированных запросов для работы с реляционными базами данных.

Плохой (опасный) пример:

```go
// Уязвимость SQL injection — НЕ делать так
...
```go
// Prepared statement — безопасно
row := db.QueryRow("SELECT id, email FROM users WHERE name = ?", userInput)
```
---
Помимо прямой работы с данными внутри информационной системы помните, что и сама ее работа будет создавать данные, своеобразный журнал происходящих событий - логи. Логгирование и трасировка есть не что иное как строки, содержащие ключевую для разработчиков информацию и обеспечивающие такое свой ство системы на "наблюдаемость".


```go
log.Printf("user=%s action=%s status=%d", userID, action, status)
```
---

В файловой системе — пути, имена, содержимое многих структурированных и полуструктурированных файлов - строки

```go
data, _ := os.ReadFile("config.yaml")
cfg := string(data) // разбор YAML затем
```
---

Внутренняя логика, системы поиска... список аспектов информационных систем, где вы столкнетесь со строками можно продолжать до бесконечноти.

# Представление строкового типа данных

Важно чётко разделять представление **метаданных (information/metadata)** и **реальных данных (data)** для типов в рантайме Go: это два разных уровня абстракции, у каждого — своя роль в системе типов. 

---

## Метаданные — цепочка описательных структур

**Метаданные** — это описательная цепочка структур, одна копия которой существует для каждого типа. Она отвечает за такие вещи, как:

* размер значения типа в байтах;
* правила копирования, сравнения и очистки;
* список полей/элементов типа;
* расположение указателей внутри значения (важно для сборщика мусора).

Рассмотрим элементы этой цепочки.

### `rtype`

Файл: `go/src/runtime/type.go`
`rtype` — базовая структура, описывающая метаданные любого типа в рантайме:
...
### `Type` (встроенная в `rtype`)

Файл: `go/src/internal/abi/type.go`
`Type` содержит ключевые метаданные о типе:

```go
type Type struct {
    Size_       uintptr // размер объекта в байтах
    PtrBytes    uintptr // количество начальных байт, содержащих указатели (для GC)
    Hash        uint32  // хеш типа (ускоряет операции в хеш-таблицах)
    TFlag       TFlag   // дополнительные флаги
    Align_      uint8   // выравнивание переменной этого типа
    FieldAlign_ uint8   // выравнивание поля в структуре с этим типом
    Kind_       Kind    // вид типа (enum)

    // Функция сравнения двух объектов этого типа:
    Equal func(unsafe.Pointer, unsafe.Pointer) bool

    // Данные для сборщика мусора: битовая маска или указатель на неё.
    GCData    *byte
...
* `GCData` обычно указывает на маску, объясняющую, какие поля содержат указатели. При наличии флага `TFlagGCMaskOnDemand` сам `GCData` может быть байтом, а реальная маска — по одному уровню разыменования ниже; рантайм при необходимости соберёт маску динамически.
* `Equal` — универсальная функция сравнения для данного типа.

### `Kind`

`Kind` — это `uint8`-перечисление, которое кодирует конкретный вид типа (например, `Int`, `Slice`, `Struct` и т.д.):
...
## Представление данных строки в рантайме

**Данные** — это сами значения, с которыми работает программа во время выполнения. Для строки представление простое и компактное.

### `stringStruct`

Файл: `go/src/runtime/string.go`
Структура строки содержит два поля — указатель на байты и длину:
...
* **Метаданные** (rtype → Type → и т. п.) описывают *что* тип представляет: размер, выравнивание, где находятся указатели, как сравнивать и т.д. — и существуют по одному экземпляру на тип.
* **Данные** (например, `stringStruct`) — это фактическое содержимое, с которым оперирует программа в рантайме.
...
В Go строка *не гарантирует*, что каждый элемент представляет символ: Go **не хранит строки как массив рун**, а хранит их как **UTF-8 байты**.

Свойства:
* **Неизменяема** (immutable)
* Содержит UTF-8 байты
* Длина `len(str)` — число байтов, а не символов
* Индексация `str[i]` возвращает **байт**, не руну

```go
s := "Привет"
fmt.Println(len(s)) // 12, потому что каждый русский символ = 2 байта в UTF-8
fmt.Println(s[0])   // байт, не символ
```

---

`rune` — это псевдоним для `int32`.
Руна представляет **один Unicode-кодпоинт**, то есть один символ в абстрактном смысле (но не обязательно «букву», т.к. бывают составные символы).

Свойства:
* Тип: `int32`
* Представляет Unicode-кодовую точку
* Используется для корректной работы с символами (а не байтами)

```go
var r rune = 'Ж'
fmt.Println(r)        // 1046 (код Unicode)
fmt.Printf("%c\n", r) // Ж
```

>Один «символ» на экране может состоять из **нескольких рун** (например, буква + диакритический знак). Поэтому «руна ≠ символ пользователя».

---

Срез байтов — это **мутируемая (изменяемая) последовательность байтов**.
Используется для работы с низкоуровневыми представлениями данных — сетевыми пакетами, файлами, буферами, и *в том числе* строками.

Свойства:
* Изменяем
* Представляет байты (математика та же, что и в `string`, но без ограничения на неизменность)
* Можно быстро создавать из строки: `[]byte(s)`
* Используется, когда нужна производительность


```go
s := "hello"
b := []byte(s)
b[0] = 'H' // можно изменить
fmt.Println(string(b)) // Hello
```

## Создание строк

Чтобы создать строку, можно использовать двойные кавычки или обратные кавычки. Двойные кавычки используются для создания строкового литерала, а обратные кавычки — для создания необработанного строкового литерала. 
...
Строковый литерал — это последовательность символов, заключённая в двойные кавычки или обратные кавычки. 
...
- сохраняются все символы как есть,
- игнорируются escape-последовательности (\n, \t, \", \\ и т. д.),
- можно свободно делать переносы строк.
...
```go
// Многострочная строка с использованием обратных кавычек.
// Такой способ самый удобный и читаемый.
text := `Это многострочная строка.
Она сохраняет все переносы,
...
// Работает, но менее удобен и менее эффективен.
text2 := "Это тоже\n" +
    "многострочная строка,\n" +
    "но создана через объединение\n" +
    "нескольких строк."
...
### 4. Наибольшая палиндромная подстрока
