...
Более лаконичную версию этой функции можно создать с помощью append. Это задание предлагается выполнить самостоятельно.


# Задания

## 1. Массив/срез создание и связь
Создайте массив, содержащий дни недели, и срез, оставшихся дней недели от сегодняшней даты. Выведете информацию о длине и ёмкости массива и среза.
## 2. Чтение массива и среза
На основе массива и срезу из задания 1 - выведите все дни недели, на следующей строке сегодняшний день недели, на последней строке - те дни недели, что остались (кроме сегодня).
## 3. Срез и допавление
Создайте срез на основе задания 1 - дополните его 7 "пятницами" (при необходимости создайте дополнительный срез). 

## 4. Работа над срезами

Условие: дана строка "Golang day here", состоящая из строчных и прописных букв латинского алфавита. Посчитайте заданные характеристики строки (как массива символов []byte) и выведете результат:
- Первый символ. 
- Предпоследний символ - на основе функции длины len(). 
- Первые пять символов, создав срез. 
- Всю строку, кроме последних двух символов - как срез. 
- Все символы с чётными индексами (индексация начинается с 0, поэтому символы выводятся начиная с первого). Сохраните результат в отдельный срез.
- Все символы с нечётными индексами, то есть начиная со второго символа строки. Сохраните результат в отдельный срез.
- Все символы в обратном порядке. Сохраните результат в отдельный срез.
- Все символы строки через один в обратном порядке, начиная с последнего. Сохраните результат в отдельный срез.
- Составьте срез, состоящий из созданных ранее срезов через пробел и выведете его длину. 
